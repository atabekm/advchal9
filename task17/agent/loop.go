...
const systemPrompt = `You are a helpful assistant with access to tools provided by an MCP server.
...
func Connect(ctx context.Context, transport mcp.Transport, obs Observer) (*mcp.ClientSession, []*mcp.Tool, error) {
	client := mcp.NewClient(&mcp.Implementation{Name: "bookagent", Version: "0.1.0"}, nil)
	var mu sync.Mutex
...
// Turns reports how many user messages the history holds.
...
// History returns a copy of the conversation (for /raw and tests).
func (a *Agent) History() []Message { return append([]Message(nil), a.history...) }
