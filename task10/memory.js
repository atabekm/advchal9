 ...
 * Task 8 answered it with a slice — keep what fits, delete the rest. Task 9
 * added the second answer: keep the recent turns exactly as they were said, and
 * replace everything older with prose that a model wrote about them. Task 10
 * adds the third, which is the same window with a key/value block in front of
 * it and no prose anywhere — the block is built by facts.js, and this file does
 * not know or ask what is in it.
 *
...
/* The answers. They are ordered by how much of the past survives, which is also
 * roughly the order they cost money in — with two deliberate exceptions, and the
 * exceptions are the last two tasks. `compress` costs more than `window` and is
 * supposed to buy something with the difference; `facts` costs more than either
 * and is supposed to buy something neither of them can.
 *
 * Task 9 left a claim in the comment above `select()`: that a fifth answer
 * would be a change to one function. `facts` is that fifth answer, and this is
 * what became of the claim — `select()`, `wire()`, `map()` and the
 * counterfactual, which are the four places that already knew a policy existed.
 * Everything a fact actually is lives in facts.js and nothing here asks. */
const MEMORY_POLICIES = ['none', 'full', 'window', 'compress', 'facts'];
...
class Compressor {
  ...
  constructor({ counter, facts = null, ...config } = {}) {
    if (!counter) throw new Error('The compressor needs a counter.');
    this._counter = counter;
    /* The fact store is injected for the same reason the counter is: it is part
     * of the conversation, not part of this object's opinion about the
     * conversation, and it has to survive a policy being switched away from
     * `facts` and back. Nothing here writes to it. */
    this._facts = facts;
  ...
  reset() {
    this._folded = 0;
    this._chain.clear();
    // The facts are part of the conversation, so they go when it goes. A fact
    // store that outlived its messages would be a set of claims about a
    // conversation it never saw, in the system slot of every future request.
    if (this._facts) this._facts.reset();
    this._spent = { folds: 0, tokens: 0, cost: 0, failures: 0 };
  }
  ...
  select(messages = [], { model = null } = {}) {
    const list = Array.isArray(messages) ? messages : [];
    const { policy, keepRecent } = this._config;
    const none = { summary: null, facts: null, folded: 0, pending: 0 };

    if (policy === 'none') {
      return {
        policy, ...none, verbatim: [],
    ...
    if (policy === 'full') {
      return {
        policy, ...none, verbatim: list.slice(),
    ...
    if (policy === 'window') {
      ...
      return {
        policy, ...none, verbatim: list.slice(from),
        dropped: from, held: list.length,
      };
    }

    /* facts: the same window as `window`, with a key/value block in front of
     * it. The messages outside the window are gone exactly as they are under
     * `window` — this policy does not keep more of the past, it keeps a
     * different thing about the past, and `dropped` says so honestly.
     *
     * The block is built by the fact store under its own ceiling. Nothing here
     * decides what is in it, which is the whole point of the split: a policy
     * that could edit the facts on the way out would be a summariser with
     * extra steps. */
    if (policy === 'facts') {
      const from = alignToTurn(list, Math.max(0, list.length - keepRecent));
      const block = this._facts ? this._facts.block(model) : null;
      return {
        policy,
        summary: null,
        facts: block && block.text ? block : null,
        folded: 0,
        pending: 0,
        verbatim: list.slice(from),
        dropped: from,
        held: list.length,
      };
    }
  ...
  wire({ messages = [], systemPrompt = '', next = '', model = null } = {}) {
    const selection = this.select(messages, { model });
    const payload = [];

    if (systemPrompt) payload.push({ role: 'system', content: systemPrompt });
    /* The facts block is a system message for the same reason the summary is:
     * nobody said it. It is a record *about* what was said, and giving it a
     * speaker would put words in the user's mouth that the user would then be
     * told they had used. */
    if (selection.facts) {
      payload.push({ role: 'system', content: selection.facts.text });
    }
  ...
  map(messages = []) {
    ...
    /* Under `facts` there are two ways to be outside the window and they are
     * not the same event: a message that put something in the store is still
     * speaking, in four words, and a message that did not is simply gone. The
     * map is the only place that difference is visible, and hiding it would
     * let the policy take credit for every message it deleted. */
    const cited = selection.policy === 'facts' && this._facts
      ? this._facts.sources()
      : null;
    return list.map((message, index) => {
      let state = 'dropped';
      if (index >= verbatimFrom) state = 'sent';
      else if (selection.policy === 'compress') state = 'summarised';
      else if (cited && cited.has(index)) state = 'distilled';
      return { index, role: message.role, state };
    });
  }
  ...
  counterfactual({ messages = [], systemPrompt = '', next = '', model, maxTokens = 0 } = {}) {
    ...
    const selection = this.select(list, { model });
    ...
    return {
      ...
      summaryTokens: selection.summary
        ...
        : 0,
      factsTokens: selection.facts
        ? this._counter.countMessage({ content: selection.facts.text }, model)
        : 0,
  ...
  snapshot() {
    return {
      ...
      spent: { ...this._spent },
      /* The facts travel with the summary, in one field, because they are the
       * same half of the conversation: the part nobody said but everybody is
       * reasoning from. A session restored without them goes back to answering
       * from a six-message window and does not mention it. */
      facts: this._facts ? this._facts.snapshot() : null,
    };
  }
  ...
  restore(snapshot) {
    ...
    if (this._facts) this._facts.restore(snapshot.facts);
