 ...
 *   agent.facts               the fact store, same reason
...
const AGENT_SCHEMA = [
  ...
  },
  {
    ...
    label: 'memory',
    default: 'facts',
    options: ['none', 'full', 'window', 'compress', 'facts'],
    help: 'none is stateless. full resends everything. window keeps the last few '
      ...
      + 'facts keeps the last few and a key/value block of what was said, quoted.',
  },
  {
    key: 'keepRecent',
    type: 'number',
    ...
    help: 'Messages kept exactly as they were said. Everything older is summarised '
      + 'under compress, deleted under window, and under facts deleted too — '
      + 'minus whatever of it reached the store as a quoted value.',
  },
  {
    key: 'compressEvery',
    type: 'number',
  ...
  },
  {
    ...
    default: 'deepseek-flash',
    options: ['deepseek-flash', 'deepseek-v4-pro', 'stub-16k'],
    help: 'Extraction is a second request, once per user message, and it is billed. '
      + 'It reads one message and the key list, never the conversation.',
  },
  {
    key: 'factsBudget',
    type: 'number',
    ...
    max: 1024,
    step: 32,
    help: 'Tokens the facts block may occupy in every request. A store with no '
      + 'ceiling grows into the history it replaced, slowly.',
  },
  {
    key: 'historyBudget',
    type: 'number',
...
class Agent {
  ...
  constructor(options = {}) {
    const { transport, onEvent, counter, compressor, facts, ...config } = options;
    ...
    this._compressor = compressor instanceof Compressor
      ...
      : new Compressor({
        ...
        facts: facts instanceof FactStore ? facts : new FactStore({ counter: this._counter }),
      });
    ...
    this._stats = {
      ...
      foldCost: 0,
      foldElapsed: 0,
      // Extraction is billed the same way and kept apart for the same reason.
      // It happens once per user message rather than once per ten, which is
      // the number the compare table has to survive.
      extractions: 0,
      extractFailures: 0,
      extractTokens: 0,
      extractCost: 0,
  ...
  _syncCompressor() {
    ...
    if (this._compressor.facts) {
      this._compressor.facts.configure({ factsBudget: this._config.factsBudget });
    }
  }
  ...
  _carried(selection) {
    ...
    if (selection.facts) {
      carried.unshift({ role: 'system', content: selection.facts.text });
    }
    return carried;
  }
  ...
  plan(text = '') {
    const selection = this._compressor.select(this._messages, { model: this._config.model });
  ...
  reset() {
    ...
    const knownFacts = this._compressor.facts ? this._compressor.facts.size : 0;
    this._messages = [];
    this._turn = 0;
    this._ledger.clear();
    ...
    this._emit('reset', { forgotten, generations, facts: knownFacts });
  }
  ...
  restore(snapshot) {
    ...
    this._emit('agent:restored', {
      ...
      facts: this._compressor.facts ? this._compressor.facts.size : 0,
  ...
  _assemble(text) {
    ...
    const selection = this._compressor.select(this._messages, { model });
    ...
    const reasons = [];
    /* The facts block is held out of the slicing for the same reason the
     * summary is, and one more: it is the only thing in the payload that is
     * already at its ceiling. Trimming it to make room would be cutting the
     * part that was chosen to fit. */
    const preamble = [];
    if (summary) preamble.push({ role: 'system', content: summary.text });
    if (facts) preamble.push({ role: 'system', content: facts.text });
    const withMemory = (list) => (preamble.length ? [...preamble, ...list] : list);
    ...
    let plan = this._planFor(withMemory(kept), text);
    ...
    if (plan.overflow) {
      ...
      const summaryTokens = summary
        ...
        : 0;
      const factsTokens = facts
        ? this._counter.countMessage({ content: facts.text }, model)
        : 0;
      const room = plan.limit - plan.reserved - plan.system - plan.next
        - plan.priming - summaryTokens - factsTokens;
      ...
      if (policy === 'trim') {
        ...
        plan = this._planFor(withMemory(kept), text);
      }
    ...
    this._emit('tokens:preflight', {
      ...
      facts: facts ? facts.included.length : 0,
      factsDropped: facts ? facts.dropped : 0,
    ...
    const messages = (dropped > 0)
      ? [
        ...
        ...(facts ? [{ role: 'system', content: facts.text }] : []),
        ...(summary ? [{
          role: 'system',
  ...
  /* Extraction: one request, one turn, after every user message.
   *
   * One turn, not one message. The brief asks for a block holding the
   * important data *of the dialogue* — goals, constraints, preferences,
   * decisions, agreements — and an agreement is two-sided by definition: half
   * of what gets settled in a requirements conversation is settled in the
   * reply. Reading only the user's half keeps half of an agreement and calls
   * it the agreement.
   *
   * What that reopens is the risk this whole file exists to close: a sentence
   * the model invented, quoted back to it later as something established. It
   * is closed again by provenance rather than by omission — every fact records
   * which of the two messages it came from, the block marks the ones the agent
   * settled, and the contract forbids taking anything the reply was merely
   * offering or guessing at.
   *
   * It runs *after* the turn has been answered and remembered, not before, and
   * the ordering is load-bearing in two ways. A fact cites the message it came
   * from by index, so it may only be written once that message has an index —
   * otherwise a turn that fails at the transport would leave the store quoting
   * a message the conversation never kept. And now there is a second reason:
   * before the transport answers, half of the turn does not exist yet.
   *
   * Like a fold, it is the plainest request the agent makes: temperature zero
   * because extraction is not a creative act, no streaming because nobody is
   * reading it, thinking off because reasoning is billed at output rates out of
   * the same ceiling the JSON has to fit in.
   *
   * Like a fold, it cannot cost a message. Everything it can do is write to a
   * store that is additive to the window; a failure leaves the previous facts
   * standing, the next turn goes out with a slightly staler block, and the log
   * says so.
   */
  async _maybeExtract({ said, turn }) {
    if (this._config.memoryPolicy !== 'facts') return null;
    const store = this._compressor.facts;
    if (!store) return null;

    const model = this._config.factsModel;
    const request = store.request({ said, turn });
    const started = Date.now();

    this._emit('facts:extract', {
      model,
      turn,
      known: store.size,
      estimatedPrompt: this._counter.estimate(`${request.system}\n${request.user}`, model),
    });

    let result;
    try {
      result = await this._transport.send({
        model,
        messages: [
      ...
      this._stats.extractFailures += 1;
      this._emit('facts:failed', {
        turn,
        message: error.message,
        known: store.size,
        kept: true,
    ...
    this._stats.extractElapsed += (Date.now() - started) / 1000;
    this._stats.extractTokens += (usage.promptTokens || 0) + (usage.completionTokens || 0);
    if (typeof result.cost === 'number') {
      this._stats.extractCost += result.cost;
      // Into the headline total as well, for the same reason a fold is: the
      // money left the account whether or not it bought an answer.
      this._stats.cost += result.cost;
    }

    /* The two sources are fixed here and the extractor may only choose between
     * them. It cannot name a message of its own: an extractor free to do that
     * could cite one that says what it wishes had been said, and the verbatim
     * rule would check the quote against the wrong message and pass it. */
    const candidates = FactStore.parse(result.text);
    const sources = {};
    for (const entry of said) {
      sources[entry.role] = { text: entry.text, index: entry.index, turn };
    }
    const { written, rejected } = store.apply(candidates, { sources });

    this._stats.factsWritten += written.length;
    this._stats.factsRejected += rejected.length;
    this._stats.factsFromAgent += written
      .filter((entry) => entry.fact.source.role === 'assistant').length;

    this._emit('facts:written', {
      turn,
      model,
      ...
      written: written.map((entry) => ({
        ...
        from: entry.fact.source.role,
        outcome: entry.outcome,
        previous: entry.previous || null,
      })),
      rejected: rejected.map((entry) => ({ key: entry.key || null, value: entry.value || null, reason: entry.reason })),
      known: store.size,
      spentTokens: (usage.promptTokens || 0) + (usage.completionTokens || 0),
  ...
  async send(text, { onChunk, signal } = {}) {
    ...
    // The second model only matters when there is going to be a second request.
    const used = ['model'];
    if (this._config.memoryPolicy === 'compress') used.push('summaryModel');
    if (this._config.memoryPolicy === 'facts') used.push('factsModel');
    for (const key of used) {
      const name = this._config[key];
      const known = MODELS[name];
      if (known && known.stub && this._transport.id !== 'echo') {
        throw new Error(
    ...
    try {
      ...
      this._emit('memory:sent', {
        ...
        facts: selection.facts ? selection.facts.included.length : 0,
        factsDropped: selection.facts ? selection.facts.dropped : 0,
        factsTokens: selection.facts ? selection.facts.tokens : 0,
      ...
      const askedAt = this._messages.length;
      this._messages.push({ role: 'user', content, at: started });
      this._messages.push({ role: 'assistant', content: result.text, at: Date.now() });

      // The answer is already in front of the person who asked for it. This is
      // the bill for the turn after next.
      await this._maybeExtract({
        turn: this._turn,
        said: [
          { role: 'user', text: content, index: askedAt },
          { role: 'assistant', text: result.text, index: askedAt + 1 },
        ],
      });
